(function () {
  ...
  angular
    .module('app')
    .controller('resellerViewCtrl', resellerViewCtrl);

  resellerViewCtrl.$inject = ['$state', '$filter', '$scope', 'logger', 'resellerFactory' , '$rootScope', 'NgTableParams', '$stateParams', '$window' ];
  /* @ngInject */
  function resellerViewCtrl($state, $filter, $scope, logger, resellerFactory , $rootScope, NgTableParams, $stateParams, $window ) {
    ...
    self.resellerId = $stateParams.id;
    ...
    function activate() {
      ...
      resellerFactory.getResellerById(self.resellerId)
          .then(function(response) {  
               self.reseller = response.data; 
               console.log(self.reseller);                
               
          },function() {
              logger.error("Something went wrong")       
          });

      resellerFactory.getOrganizations()
          .then(function(response) {  
      ...
      self.tableParams = new NgTableParams({
          ...
          getData : function(params){          
            if(self.organizations!=null){ 
                ...
                if (params != null) {
                if (params.filter()) {
                    filteredData = $filter('filter')(self.organizations, params.filter())
                  }
                  else {
                    filteredData = self.organizations.content;
                  }
                  if (params.sorting()) {
                  ...
                  return self.organizations.content;
